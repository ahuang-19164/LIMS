...
namespace Wx.EntryInfo
{
    public partial class FrmPerworkInfoCheck : DevExpress.XtraEditors.XtraForm
    {
        ...
        public FrmPerworkInfoCheck()
        {
            ...
            SampleInfoCheck = ConfigInfos.ReadConfigInfo("SampleInfoCheckWx");
        ...
        {
            string FileDirectory = Application.StartupPath + "\\CheckTemplateInfo";
        ...
        private void CEEntry_CheckedChanged(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (CEEntry.Checked)
            {
                CEExcel.Checked = false;
                CEInterface.Checked = false;
                EntryTableName = "WorkPer.SampleInfo";
            }
        }

        private void CEExcel_CheckedChanged(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (CEExcel.Checked)
            {
                CEEntry.Checked = false;
                CEInterface.Checked = false;
                EntryTableName = "WorkPer.SampleInfoExcel";
            }
        }
        private void CEInterface_CheckedChanged(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (CEExcel.Checked)
            {
                CEEntry.Checked = false;
                CEExcel.Checked = false;
                EntryTableName = "WorkPer.SampleInfoInterface";
            }
        }
        ...
        private void BTSampleInfoEdit_Click(object sender, EventArgs e)
        {
            ...
            //}
            bool sampleState = true;//判断是否用check字段进行审核
            //int editState = 1;
            for (int a = GVSampleInfo.RowCount-1; a >= 0; a--)
            {
                if (GVSampleInfo.GetDataRow(a)["check"] != DBNull.Value)
                {
                    ...
                    {
                        string perStateNO = GVSampleInfo.GetDataRow(a)["perStateNO"] != DBNull.Value ? GVSampleInfo.GetDataRow(a)["perStateNO"].ToString() : "1";
                        if (perStateNO == "1")
                        {
            ...
            if (sampleState)
            {
                FrmSampleInfoCheckEdit frmSampleInfoEdit = new FrmSampleInfoCheckEdit(data, 1,1);
                frmSampleInfoEdit.ShowDialog();
            }
            else
            {
                FrmSampleInfoEdit frmSampleInfoEdit = new FrmSampleInfoEdit(data,1);
                frmSampleInfoEdit.ShowDialog();
            }
        }
                    ...
                    if (GVSampleInfo.GetDataRow(a)["check"] != DBNull.Value)
                    {
                        if (Convert.ToBoolean(GVSampleInfo.GetDataRow(a)["check"]))
                        {
                            ...
                            if (GVSampleInfo.GetDataRow(a)["barcode"] != DBNull.Value)
                            {
                                ...
                                {
                                    if (GVSampleInfo.GetDataRow(a)["perStateNO"].ToString() == "5")
                                    {
                                        ...
                                        //{
                                            ...
                                            uInfo.value = "perStateNO='1'";
                                            uInfo.wheres = $"barcode='{GVSampleInfo.GetDataRow(a)["barcode"]}'";
                                            uInfo.ConnType = 1;
                                            uInfo.MessageShow = 1;
                                        ...
                                        //    return ApiHelpers.postInfo(SampleInfoCheck, Sr);
                                        //});
                                        //task.Start();
                                        //task.Wait();
                                        //string s = task.Result;
                                        //CheckCodeModel CheckCodeModel = JsonHelper.JsonConvertObject<CheckCodeModel>(jm);
                                        ////CheckCodeModel CheckCodeModel = JsonHelper.JsonConvertObject<CheckCodeModel>(jm);
                                        //if (CheckCodeModel.code == 1)
                                        //{
                                        //    GVSampleInfo.SetRowCellValue(a, "perStateNO", 2);
                                        //    GVSampleInfo.SetRowCellValue(a, "checker", CommonData.UserInfo.names);
                                        //    GVSampleInfo.SetRowCellValue(a, "checkTime", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                                        //    if (CEPrintState.Checked)
                                        //    {
                                        //        try
                                        //        {
                                        //            foreach (CheckGroupBarcode groupBarcode in CheckCodeModel.groupcodeInfo)
                                        //            {
                                        //                getPrintBarcodeInfo(a, groupBarcode);
                                        //            }
                                        //        }
                                        //        catch (Exception ex)
                                        //        {
                                        ...
                                        //else
                                        //{
                                        //    resultInfo += CheckCodeModel.msg;
                                        //}
                                    }
                                    else
                                    {
                                        resultInfo += GVSampleInfo.GetDataRow(a)["barcode"] + "样本信息不能审核。\r\n";
                                    }
                                }
                                else
                                {
                    ...
                    if (GVSampleInfo.GetDataRow(a)["check"] != DBNull.Value)
                    {
                        if (Convert.ToBoolean(GVSampleInfo.GetDataRow(a)["check"]))
                        {
                            ...
                            if (GVSampleInfo.GetDataRow(a)["barcode"] != DBNull.Value)
                            {
                                if (GVSampleInfo.GetDataRow(a)["perStateNO"] != DBNull.Value)
                                {
                                    if (GVSampleInfo.GetDataRow(a)["perStateNO"].ToString() == "1" || GVSampleInfo.GetDataRow(a)["perStateNO"].ToString() == "5")
                                    {
                                        ...
                                        commReInfo<ReCheckeModel> CheckCodeModel = JsonHelper.JsonConvertObject<commReInfo<ReCheckeModel>>(jm);
        ...
        private void getPrintBarcodeInfo(int selectRowNO, CheckGroupBarcode groupBarcode)
        {
            if (GVSampleInfo.GetDataRow(selectRowNO)["barcode"] != DBNull.Value)
            {
                groupBarcode.barcode = GVSampleInfo.GetDataRow(selectRowNO)["barcode"].ToString();
            }
            if (GVSampleInfo.GetDataRow(selectRowNO)["hospitalNames"] != DBNull.Value)
            {
                groupBarcode.hospitalNames = GVSampleInfo.GetDataRow(selectRowNO)["hospitalNames"].ToString();
            }
            if (GVSampleInfo.GetDataRow(selectRowNO)["patientName"] != DBNull.Value)
            {
                groupBarcode.patientName = GVSampleInfo.GetDataRow(selectRowNO)["patientName"].ToString();
            }
            if (GVSampleInfo.GetDataRow(selectRowNO)["patientSexNames"] != DBNull.Value)
            {
                groupBarcode.patientSexNames = GVSampleInfo.GetDataRow(selectRowNO)["patientSexNames"].ToString();
            }
            if (GVSampleInfo.GetDataRow(selectRowNO)["ageYear"] != DBNull.Value)
            {
                groupBarcode.ageYear = GVSampleInfo.GetDataRow(selectRowNO)["ageYear"].ToString();
            }
            //if (groupBarcode.groupNO != null&& groupBarcode.groupName==null)
            //{
            ...
            //        {
            //            groupBarcode.groupName=rows[0]["names"].ToString();
            //        }
            //    }
            //}
        ...
        string InfoBarcode = "";
        private void GVItemInfo_Click(object sender, EventArgs e)
        {
        ...
        private void RTEbarcodes_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)//这是允许输入退格键
            {
                ...
                if (textEdit.EditValue != null)
                {
                    ...
                    {
                        CheckSelectModel checkModel = new CheckSelectModel();
                        string assss= TEbarcodes.EditValue != null && TEbarcodes.EditValue.ToString().Trim().Length > 0 ? TEbarcodes.EditValue.ToString() : null;
                        if(assss != null)
                        {
        ...
        private void TEbarcodes_DoubleClick(object sender, EventArgs e)
        {
                            ...
                            if (GVSampleInfo.GetDataRow(a)["barcode"] != DBNull.Value)
                            {
                                if (GVSampleInfo.GetDataRow(a)["perStateNO"] != DBNull.Value)
                                {
                                    if (GVSampleInfo.GetDataRow(a)["perStateNO"].ToString() == "2")
                                    {
                                        ...
                                        commReInfo<commReSampleInfo> CheckCodeModel = JsonHelper.JsonConvertObject<commReInfo<commReSampleInfo>>(jm);
                                        if (CheckCodeModel.code == 1)
                                        {
                                            GVSampleInfo.SetRowCellValue(a, "perStateNO", 1);
                    ...
                    if (GVSampleInfo.GetDataRow(a)["check"] != DBNull.Value)
                    {
                        if (Convert.ToBoolean(GVSampleInfo.GetDataRow(a)["check"]))
                        {
                            ...
                            if (GVSampleInfo.GetDataRow(a)["barcode"] != DBNull.Value)
                            {
                                ...
                                commReInfo<CheckGroupBarcode> CheckCodeModel = JsonHelper.JsonConvertObject<commReInfo<CheckGroupBarcode>>(jm);
                                if (CheckCodeModel.code == 1)
                                {
                                    foreach (CheckGroupBarcode groupBarcode in CheckCodeModel.infos)
                                    {
                                        printBarcode(groupBarcode);
                                    }
                                }
                                ...
                                {
                                    MessageBox.Show(CheckCodeModel.msg, "系统提示");
                                }
                            }
                        }
                    }
        ...
        private void BTDeletes_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                try
                {
                            ...
                            if (Convert.ToBoolean(GVSampleInfo.GetDataRow(a)["check"]))
                            {
                                ...
                                if (GVSampleInfo.GetDataRow(a)["barcode"] != DBNull.Value)
                                {
                                    if (GVSampleInfo.GetDataRow(a)["perStateNO"] != DBNull.Value)
                                    {
                                        ...
                                        {
                                            resultInfo += GVSampleInfo.GetDataRow(a)["barcode"] + "样本信息不能删除。\r\n";
                                        }
                                    }
                                    else
                                    {
                    ...
                    //                    Task<string> task = new Task<string>(() =>
                    //                    {
                    //                        sampleInfoList info = new sampleInfoList();
                    //                        if (GVSampleInfo.GetDataRow(a)["id"] != DBNull.Value)
                    //                        {
                    //                            info.SampleID = GVSampleInfo.GetDataRow(a)["id"].ToString();
                    //                        }
                    ...
                    //                        deleteInfos.sampleinfos = infoLists;
                    //                        string Sr = JsonHelper.SerializeObjct(deleteInfos);
                    //                        return ApiHelpers.postInfo(SampleInfoDelete, Sr);
                    //                    });
                    //                    task.Start();
                    //                    task.Wait();
                    //                    string s = task.Result;
