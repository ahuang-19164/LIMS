...
{
    public partial class FrmTestOtherInfo : XtraForm
    {
        public FrmTestOtherInfo()
        {
            InitializeComponent();
        ...
        private void BTSelect_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            string wheres = $" createTime>='{DEstartTime.EditValue}' and createTime<='{Convert.ToDateTime(DEendTime.EditValue).AddDays(+1).ToString("yyyy-MM-dd")}'  and recordTypeNO not in ('1','2') and dstate=0";
        ...
        private void BTHandleInfo_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (BTHandleInfo.Enabled)
            {
                if (GVdelegateInfo.GetFocusedDataRow() != null)
                {
                    ...
                    //if(recordTypeNO=="1")
                    //{
                    FrmTestHandle frmTestHandle = new FrmTestHandle(sampleInfo);
