        ...
        private void FrmShowItem_Load(object sender, EventArgs e)
        {
            ...
            if (WorkCommData.DTItemGroup != null)
            {
                if (itemlist != "")
                {
                    ...
                    {
                        DataTable dataTable = WorkCommData.DTItemGroup.Select($"ihcState=0 and state=1 and dstate=0 and no not in ({itemlist})").CopyToDataTable();
                ...
                else
                {
                    ...
                    {
                        DataTable dataTable = WorkCommData.DTItemGroup.Select($"ihcState=0 and state=1 and dstate=0").CopyToDataTable();
                        dataTable.Columns.Add("check", typeof(bool));
                        GCGroupInfo.DataSource = dataTable;
                    }
                }
