...
namespace WorkComm.TestInfo
{
    ...
    public partial class FrmSampleTestXG : DevExpress.XtraEditors.XtraForm
    {
        ...
        public FrmSampleTestXG()
        {
            ...
            TestSampleInfoSelect = ConfigInfos.ReadConfigInfo("TestSampleInfoSelect");
        ...
        private void GVSampleInfo_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            ...
            try
            {
                ...
                if (SampleInfoRow != null)
                {
                    ...
                    //TEchecker.EditValue = SampleInfoRow["checker"];
                    //DEcheckTime.EditValue = SampleInfoRow["checkTime"] != DBNull.Value ? Convert.ToDateTime(SampleInfoRow["checkTime"]).ToString("yyyy-MM-dd HH:mm:ss") : null;
                        ...
                        if (groupFlowNOs != groupFlowNO)
                        {
                            ...
                            if (WorkCommData.DTItemFlow.Select($"no='{groupFlowNO}'").Count() > 0)
                            {
                                ...
                                if (!Convert.IsDBNull(dataRow["reflectionFile"]) && !Convert.IsDBNull(dataRow["reflectionFrm"]))
                                {
                                    ...
                                    reflectionFile = "WorkTest.TestXG";
                                    reflectionFrm = "FrmTestXG";
        ...
        private void BTSampleSelect_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (GESelectGroup.EditValue != null)
            {
                ...
                WebApiCallBack jm = ApiHelpers.postInfo(TestSampleInfoSelect, Sr);
                ...
                //if (GESampleSate.EditValue.ToString() == "0")
                //{
                //    selectInfo.wheres = $" (reachTime between '{Convert.ToDateTime(DEStarTime.EditValue).ToString("yyyy-MM-dd")}' and '{Convert.ToDateTime(DEEndTime.EditValue).AddDays(+1).ToString("yyyy-MM-dd")}') and state=1 and dstate=0  and groupNO='{GESelectGroup.EditValue}'";
                //}
                //else
                //{
                //    selectInfo.wheres = $" (reachTime between '{Convert.ToDateTime(DEStarTime.EditValue).ToString("yyyy-MM-dd")}' and '{Convert.ToDateTime(DEEndTime.EditValue).AddDays(+1).ToString("yyyy-MM-dd")}') and state=1 and dstate=0 testStateNO='{GESampleSate.EditValue}' and groupNO='{GESelectGroup.EditValue}'";
                //}
                //selectInfo.OrderColumns = "urgent desc,createTime";

                //DTTestInfo = ApiHelpers.postInfo(selectInfo);
                //GCSampleInfo.DataSource = DTTestInfo;
                //GVSampleInfo.BestFitColumns();
                //testState();
        ...
        public bool SaveResult(int testStateNO, int typeNO, out string resultString)
        {
            ...
            resultString = "";//返回执行信息
                              ...
                              //string itemString = SampleInfoRow["groupFlowNO"] != DBNull.Value ? SampleInfoRow["groupFlowNO"].ToString() : "";
                              //判断是否包含融合码信息
                                    ...
                                    if (autographInfo.testTime > receiveTime)
                                    {
                                        ...
                                        if (saveState)
                                        {
                                            ...
                                            if (autographInfo.testTime <= DateTime.Now)
                                            {
                                                ...
                                                object[] paras = new object[] { testStateNO, perid, testid, sampleid, barcodeList, groupNO, groupFlowNO, autographInfo };
                                                string ReturnState = postResultInfos.Invoke(objpostResultInfo, paras).ToString();
                                                if (ReturnState != "")
                                                {
                                                    commReInfo<commReItemInfo> commReInfo = JsonHelper.JsonConvertObject<commReInfo<commReItemInfo>>(ReturnState);
                                            ...
                                            if (checkTime <= DateTime.Now)
                                            {
                                                if (testTime < Convert.ToDateTime(DEcheckTime.EditValue))
                                                {
                                                    ...
                                                    if (typeNO == 1)
                                                    {
                                                        ...
                                                        if (autographInfo.checkTime < autographInfo.testTime.AddHours(+1))
                                                        {
                                                            DialogResult dialogResult = MessageBox.Show("编辑审核时间与编辑检测时间未超过一小时，是继续否保存？", "系统提示", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                                                            if (dialogResult == DialogResult.Yes)
                                                            {
                                                ...
                                                if (autographInfo.tester != autographInfo.checker)
                                                {
                                                    if (testTime < Convert.ToDateTime(DEcheckTime.EditValue))
                                                    {
                                                        ...
                                                        if (typeNO == 1)
                                                        {
                                                            if (autographInfo.checkTime < autographInfo.testTime.AddHours(+1))
                                                            {
                                                                DialogResult dialogResult = MessageBox.Show("编辑审核时间与编辑检测时间没有超过一小时，是否保存？", "系统提示", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                                                                if (dialogResult == DialogResult.Yes)
                                                                {
                                                                    //resultString = "编辑审核时间与编辑检测时间没有超过一小时,允许保存";
                                                                    saveState = true;
                            ...
                            if (autographInfo.testTime > receiveTime)
                            {
                                ...
                                if (testStateNO == 3)
                                {
                                    if (typeNO == 1)
                                    {
                                        ...
                                        if (autographInfo.checkTime < autographInfo.testTime.AddHours(+1))
                                        {
                                            DialogResult dialogResult = MessageBox.Show("审核时间与检测时间没有超过一小时，是否保存？", "系统提示", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                                            if (dialogResult == DialogResult.Yes)
                                            {
            ...
            if (GVSampleInfo.GetSelectedRows().Length > 0)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    for (int w = a.Length - 1; w >= 0; w--)
                    {
                        ...
                        Barcode = SampleInfoRow["barcode"] != DBNull.Value ? SampleInfoRow["barcode"].ToString().Trim().ToUpper() : "";
                        ReceiveSampleInfoModel receiveInfo = new ReceiveSampleInfoModel();
                        receiveInfo.perid = Convert.ToInt32(SampleInfoRow["perid"]);
                        receiveInfo.testid = Convert.ToInt32(SampleInfoRow["id"]);
                        receiveInfo.barcode = SampleInfoRow["barcode"] != DBNull.Value ? SampleInfoRow["barcode"].ToString() : "";
                        receiveInfo.testNo = SampleInfoRow["testNo"] != DBNull.Value ? SampleInfoRow["testNo"].ToString() : "";
                        receiveInfo.frameNo = SampleInfoRow["frameNo"] != DBNull.Value ? SampleInfoRow["frameNo"].ToString() : "";
                        receiveInfos.Add(receiveInfo);
                    }
                    if (receiveInfos.Count > 0)
                    {
            ...
            if (setResultInfos != null && postResultInfos != null)
            {
                ...
                try
                {
                    ...
                    if (barcodeList != "")
                    {
                        ...
                        peridList = peridList.Substring(0, peridList.Length - 1);
                        testidList = testidList.Substring(0, testidList.Length - 1);
                        sampleidList = sampleidList.Substring(0, sampleidList.Length - 1);
                        resultString += SaveXG(resultString);
                    }
                    else
                    {
                        resultString = "请选择样本信息";
                    }
                    if (resultString != "")
                    {
                    ...
                    else
                    {
                        ...
                        //{
                        //    int s = a[w];
                        //    GVSampleInfo.SetRowCellValue(w, "testStateNO", 3); ;
                        //}
                    }
        ...
        private  string SaveXG(string resultString)
        {
            if (tcequal)
            {
                if (rcequal)
        ...
        private void RDEcheckTime_EditValueChanged(object sender, EventArgs e)
        {
            ...
            //    else
            //    {
            //        if (SetdateTime.AddHours(+1) > SetcheckTime)
            //        {
