...
{
    public partial class FrmTestPathologyIHC : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void FrmPathologyTest_Load(object sender, EventArgs e)
        {
            ...
            MemoEdits.AddDoubleMethod(MEihcResult);
        ...
        public void setResultInfo(int testid, DataRow SampleInfo, int TestStateNO = 0)
        {
            ...
            if (ResultTask.Result != null)
            {
                ...
                MEihcResult.EditValue = ResultTask.Result.Rows[0]["ihcResult"] != DBNull.Value ? ResultTask.Result.Rows[0]["ihcResult"] : "";
            }
            else
            {
        ...
        {
            if (MEihcResult.EditValue != null && MEihcResult.EditValue.ToString().Trim() != "")
            {
                Task<string> task = new Task<string>(() =>
                {
                    if (testid != 0 && barcode != "")
                    {
                        ...
                        if (MEihcResult.EditValue != null)
                            pathnology.ihcResult = MEihcResult.EditValue.ToString();
            ...
            {
                return "{\"code\":0,\"infos\":null,\"nextFlowNO\":\"0\",\"msg\":\"免疫组化结果不能为空。\"}";
                //MessageBox.Show("请选择需要保存信息的标本信息", "系统提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
